...
while ~isempty(NodesOpen)
    ...
    for i=1:size(Nbs)
        ...
        TempGScore =sqrt(sum(bsxfun(@minus, AllPts(CurrNode, :), AllPts(StartNode,:)).^2, 2));% + CalcH(CurrNode,NbsIdx,AllPts,astar);
        b = NodesOpen(NbsIdx);
            if ~b||TempGScore<pdist2(AllPts(NbsIdx,:),AllPts(StartNode,:))
                Prev(NbsIdx) = CurrNode;
                GScore(NbsIdx) = TempGScore;
                FScore(NbsIdx)=sqrt(sum(bsxfun(@minus, AllPts(NbsIdx, :), AllPts(StartNode,:)).^2, 2)) + CalcH(NbsIdx,GoalNode,AllPts,astar);
                if FScore(NbsIdx,:) == 0
                    FScore(NbsIdx,:) = NaN;
                end
                if b
