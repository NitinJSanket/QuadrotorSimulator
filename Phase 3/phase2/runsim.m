...
disp(['Total Position Error is ', num2str(ErrorPos')]);
disp(['Total Velocity Error is ', num2str(ErrorVel')]);

fprintf('finished.\n');

kp = [5.5 5.5 200]'; % X Y Z 50 16 good for Z
kd = [3.8 3.8 50]'; % X Y Z
kpAng = [0.1 0.1 0]'; % Roll Pitch Yaw
kdAng = [0.1 0.1 0]'; % Roll Pitch Yaw

fid = fopen('LogCircle.txt', 'at');
c = fix(clock);
fprintf(fid,'\n');
fprintf(fid,'\n');
fprintf(fid, ['Log at Time: ',num2str(c)]);
fprintf(fid,'\n');
fprintf(fid, ['Total Pos Error: ', num2str(ErrorPos')]);
fprintf(fid,'\n');
fprintf(fid, ['Total Vel Error: ', num2str(ErrorVel')]);
fprintf(fid,'\n');
fprintf(fid, ['Kp and Kd (XYZ): ', num2str([kp', kd'])]);
fprintf(fid,'\n');
fprintf(fid, ['Kp and Kd (Ang): ', num2str([kpAng', kdAng'])]);
fprintf(fid,'\n');
fclose(fid);
